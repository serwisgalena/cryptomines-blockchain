...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "kop"), encode_puzzle_hash(new_ph_2, "kop"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "kop"), encode_puzzle_hash(observer_pool, "kop")
    )
    ...
    assert config["farmer"]["xch_target_address"] == encode_puzzle_hash(observer_farmer, "kop")
    assert config["pool"]["xch_target_address"] == encode_puzzle_hash(observer_pool, "kop")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "kop")
