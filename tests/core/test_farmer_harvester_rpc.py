...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    assert config["farmer"]["kop_target_address"] == encode_puzzle_hash(observer_farmer, "kop")
    assert config["pool"]["kop_target_address"] == encode_puzzle_hash(observer_pool, "kop")
