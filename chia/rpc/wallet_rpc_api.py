...
class WalletRpcApi:
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("flo_target_address")
        pool_target = config["pool"].get("flo_target_address")
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cat_wallet":
            # If not provided, the name will be autogenerated based on the limiter hash.
